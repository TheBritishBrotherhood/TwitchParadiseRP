...
local MissionData = {
    [0] = {594.761291503906, -2770.05224609375, 6.05681467056274, 350}, --x,y,z,money
    [1] = {2574.5144, 328.5554, 108.45, 1568},
    [2] = {602.009521484375, 268.625, 103.472854614258, 1875},
    [3] = {650.879333496094, 2768.23828125, 41.9326171875, 3200},
    [4] = {2136.423828125, 4778.421875, 40.9705619812012, 3450},
...
function tick()
    ...
    if( GetDistanceBetweenCoords( playerCoords, TruckingCompany[0]["x"], TruckingCompany[0]["y"], TruckingCompany[0]["z"] ) < 10) then
            if(GUI.showStartText == false) then
                ...
                if(IsControlPressed(1, Keys["BACKSPACE"]) and GUI.showMenu == true) then
                    GUI.showMenu = false
                end
            else
                GUI.showStartText = false
    ...
    elseif( MISSION.start == true ) then
        ...
        if( IsEntityAttached(MISSION.trailer) and text1 == false) then
            TriggerEvent("mt:missiontext", "Meet at ~g~point~w~ By your GPS.", 10000)
            text1 = true
        ...
        if ( GetDistanceBetweenCoords(currentMission[1], currentMission[2], currentMission[3], trailerCoords ) < 25 and  not IsEntityAttached(MISSION.trailer)) then
            TriggerEvent("mt:missiontext", "You have earned $"..currentMission[4], 5000)
        ...
        elseif ( GetDistanceBetweenCoords(currentMission[1], currentMission[2], currentMission[3], trailerCoords ) < 25 and IsEntityAttached(MISSION.trailer) ) then
            TriggerEvent("mt:missiontext", "You have arrived. Remove the ~o~trailer~w~ By pressing ~r~H~w~", 15000)
        end
...
function MISSION.spawnTruck()
    ...
    SetVehicleEngineTorqueMultiplier(MISSION.truck, 0.3)
...
function MISSION.spawnTrailer()
    ...
    SetVehicleOnGroundProperly(MISSION.trailer)
    SetEntityInvincible(MISSION.trailer, true)
...
function GUI.drawStartText()
    TriggerEvent("mt:missiontext", "Trucking Jobs, Press ~r~E~w~ to begin.", 500)
...
function GUI.init()
    GUI.loaded = true
    GUI.addTitle("You're on the road now.", 0.425, 0.19, 0.45, 0.07 )
    GUI.addDesc("Choose your transport.", 0.575, 0.375, 0.15, 0.30 )

    --menu, title, function, position
    GUI.addButton(0, "Transport Tanker RON", GUI.optionMisson, 0.35, 0.25, 0.3, 0.05 )
    GUI.addButton(0, "Transport Container", GUI.optionMisson, 0.35, 0.30, 0.3, 0.05 )
    GUI.addButton(0, "Transprot Articulated", GUI.optionMisson, 0.35, 0.35, 0.3, 0.05 )
    GUI.addButton(0, "Transport Wood", GUI.optionMisson, 0.35, 0.40, 0.3, 0.05 )
    GUI.addButton(0, " ", GUI.null, 0.35, 0.45, 0.3, 0.05)
    GUI.addButton(0, "Exit from the Menu", GUI.exit, 0.35, 0.50, 0.3, 0.05 )
    
    GUI.buttonCount = 0
    
    GUI.addButton(1, "Mission 1 [ $350 ]", GUI.mission, 0.35, 0.25, 0.3, 0.05)
    GUI.addButton(1, "Mission 2 [ $1568 ]", GUI.mission, 0.35, 0.30, 0.3, 0.05)
    GUI.addButton(1, "Mission 3 [ $1875 ]", GUI.mission, 0.35, 0.35, 0.3, 0.05)
    GUI.addButton(1, "Mission 4 [ $3200 ]", GUI.mission, 0.35, 0.40, 0.3, 0.05)
    GUI.addButton(1, "Mission 5 [ $3450 ]", GUI.mission, 0.35, 0.45, 0.3, 0.05)
    --GUI.addButton(1, "Test [ 1$ ]", GUI.mission, 0.35, 0.50, 0.3, 0.05)
    GUI.addButton(1, "Exit from the Menu", GUI.exit, 0.35, 0.55, 0.3, 0.05)
end
--Render stuff
