...
function ACL:canConnect( netID, playerName )
	...
	if self.maxPing > 0 and png > self.maxPing then
		self.lastError = "Din ping er for høj (" .. png .. "ms / " .. self.maxPing .. "ms)."
		return false
	end
	...
	self.lastError = "Private Server > Apply here http://twitchparadise.com/"
	return false
end
...
function ACL:isBanned( playerID, playerName )
	...
	if string.upper( playerName ) == "SERVER" or string.upper( playerName ) == "USAGE" then
		self.lastError = string.format( "Navnet '%s' er reserveret. Skift navn.", playerName )
		return true
	end
	...
	for k,v in pairs( self.banlist ) do
		...
		if string.find( playerName, v, 1, true ) ~= nil then
			self.lastError = "Dit navn eller dele af det er ikke tilladt."
			return true
		end
		
		-- Check for banned individuals
		if findi( ids, v ) ~= nil then
			self.lastError = "You're banned."
			return true
		end
	end
...
OnCmd:register( "kick", function( source, args )
	...
	reason = reason ~= nil and reason or "Kicket."
	...
	OnCmd:feedback( source, "Usage", {255, 255, 255}, "/kick PlayerID [Årsag]" )
	OnCmd:feedback( source, "Usage", {255, 255, 255}, "/playerlist (Viser spillere)" )
end)
...
AddEventHandler( "playerConnecting", function( playerName, setCallback )
	...
	-- print( string.format( "'%s' forbinder. (%s)", playerName, table.concat( ident, ", " ) ) )
	...
	if #playersConnecting > 0 then
		...
		if EP ~= nil then
			...
			if ACL:isBannedIP( EP ) then
				DropPlayer( playersConnecting[1], "Du er banned." )
			end
