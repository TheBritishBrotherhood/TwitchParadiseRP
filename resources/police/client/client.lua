...
RegisterNetEvent("police:911_recvd")
AddEventHandler("police:911_recvd", function(source, caller_name, x, y, z, resultmsg)
	--Check if the player is on duty, which confirms they are a cop.
	if (getIsInService())then 
		local trace = "Call from ^5" ..  caller_name .."^1: " ..tostring(resultmsg)
		TriggerEvent("chatMessage", '112', { 0, 255, 255}, trace)
		-- Sets a blip on map
		local blip911 = AddBlipForCoord(x, y, z)
		PlaySound(-1, "BASE_JUMP_PASSED", "HUD_AWARDS", 0, 0, 1) 
		SetBlipSprite(blip911, 1)
		SetBlipColour(blip911, 1)
		SetBlipFlashesAlternate(blip911, true)
		BeginTextCommandSetBlipName("STRING")
		AddTextComponentString(resultmsg)
		EndTextCommandSetBlipName(blip911)
		
		--Set a blip, which "fade out" on the Pause-map: (255*100ms) = 25 secondes
		--Gives police time to set a marker to the specific point
		local transG = 255
		SetBlipAlpha(blip911,  transG)
		while transG ~= 0 do
			Wait(100)
			transG = transG - 1
			SetBlipAlpha(blip911,  transG)
		...
		if transG == 0 then
		    SetBlipSprite(blip911, 2) --invisible
		end
	end --isInService
end)