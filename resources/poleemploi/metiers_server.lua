...
require "resources/mysql-async/lib/MySQL"
	...
	function MineGetPlayerJobID(playerId)
		local result = MySQL.Sync.fetchAll("SELECT job FROM users WHERE identifier = @name", {['@name'] = playerId})
		return result[1].job
	end
	
	function MineGetPlayerJobName(JobId)
		local result = MySQL.Sync.fetchAll("SELECT * FROM jobs_nem WHERE job_id = @namejob", {['@namejob'] = JobId})
	...
	AddEventHandler('metiers:jobs', function(id)
		TriggerEvent('es:getPlayerFromId', source, function(user)		
			...
			MySQL.Async.execute("UPDATE users SET `job`=@value WHERE identifier = @identifier", {['@value'] = id, ['@identifier'] = user.identifier})
